DROP TABLE IF EXISTS reservas;
DROP TABLE IF EXISTS pedidosprodutositensopcoes;
DROP TABLE IF EXISTS pedidosprodutos;
DROP TABLE IF EXISTS pedidos;
DROP TABLE IF EXISTS servicos;
DROP TABLE IF EXISTS menusdiassemana;
DROP TABLE IF EXISTS diassemana;
DROP TABLE IF EXISTS itensmenus;
DROP TABLE IF EXISTS menus;
DROP TABLE IF EXISTS itensopcoes;
DROP TABLE IF EXISTS opcoes;
DROP TABLE IF EXISTS itenscategorias;
DROP TABLE IF EXISTS categorias;
DROP TABLE IF EXISTS itenstipos;
DROP TABLE IF EXISTS itens;
DROP TABLE IF EXISTS tipos;
DROP TABLE IF EXISTS produtos;
DROP TABLE IF EXISTS mesas;
DROP TABLE IF EXISTS instrucoesingredientes;
DROP TABLE IF EXISTS instrucoesutensilios;
DROP TABLE IF EXISTS instrucoes;
DROP TABLE IF EXISTS utensiliosreceitas;
DROP TABLE IF EXISTS ingredientesreceitas;
DROP TABLE IF EXISTS receitas;
DROP TABLE IF EXISTS utensilioscarrinhos;
DROP TABLE IF EXISTS utensilios;
DROP TABLE IF EXISTS ingredientescarrinhos;
DROP TABLE IF EXISTS ingredientes;
DROP TABLE IF EXISTS carrinhos;
DROP TABLE IF EXISTS fornecedores;
-- DROP TABLE IF EXISTS utilizadorespermissoes;
-- DROP TABLE IF EXISTS utilizadorescargos;
-- DROP TABLE IF EXISTS utilizadores;
-- DROP TABLE IF EXISTS permissoescargos;
-- DROP TABLE IF EXISTS cargos;
-- DROP TABLE IF EXISTS permissoes;
DROP TABLE IF EXISTS estadosmesas;